...
class Analyzer:
    ...
    def optimize(self):
        ...
        #print w
        #ppGlobals = globals()
        #job = job_server.submit(myjob,(a,w),(self.notsame,),)
        #result = job()
        #sys.exit(1)
                    ...
                    for i1 in range(-1,2):
                        for j1 in range(-1,2):
                            for k1 in range(-1,2):
                                temp=[]
                                for a in processing:
                                    temp.append(symop.newsympass(a,i1,j1,k1))
...
def run():
    ...
    try:
        ...
        return
    if(not os.path.exists('../../../results/%d'%int(myid))):
            os.makedirs('../../../results/%d'%int(myid))
    root =  'Renumbered/'
    if(root[-1:] != '/'):
        root += '/'
    url = 'http://129.10.89.145/running.php?job=%d&status=200'%int(myid)
    print url
    ...
    if (a.failC1()):
        print "Missing Cryst1 Line"
        url='http://129.10.89.145/running.php?job=%d&status=222'%int(myid)
        raw_return=urllib.urlopen(url).read()
        sys.exit(1)
    a.phase1(myid)
    print "Exiting Phase"
    url = 'http://129.10.89.145/running.php?job=%d&status=777'%int(myid)
    raw_return=urllib.urlopen(url).read()
    return
