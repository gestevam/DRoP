...
class Analyzer:
    ...
    def newgroup(self, max_err):
        ...
        file = open(self.root+'water_clusters_pass_%d.txt'%self.magi, 'w')
    ...
    def newgrouporg(self, max_err):
        ...
        if self.magi2==0:
            self.org_table=[]
            self.org_table_post=[]
        file = open(self.root+'org_clusters_pass_%d.txt'%self.magi, 'w')
    ...
    def findoutliers1(self):
        ...
        for cluster in self.master:
            ...
            if(clustersize >= 3 and clustersize <= 7):
                ...
                if (x2-x1) / (xn-x1) > critvalues[clustersize]:
            ...
            if(clustersize >= 8 and clustersize <= 10):
                ...
                xn = tempcluster[clustersize-1]['x']
                xn1 = tempcluster[clustersize-2]['x']
                if (x2-x1) / (xn1-x1) > critvalues[clustersize]:
                    # print "Critical Value 1x Fails"
                        if not tempcluster[0] in outliers:
                            outliers.append(tempcluster[0])
                if (xn-xn1) / (xn-x2) > critvalues[clustersize]:
                    #  print "Critical Value 2x Fails"
                        if not tempcluster[clustersize-1] in outliers:
                            outliers.append(tempcluster[0])
                tempcluster = sorted(cluster, key=lambda atom: atom['y'])
            ...
            if(clustersize >= 11 and clustersize <= 13):
                ...
                if (x3-x1) / (xn1-x1) > critvalues[clustersize]:
                    # print "Critical Value 1x Fails"
                        if not tempcluster[0] in outliers:
                            outliers.append(tempcluster[0])
                if (xn-xn2) / (xn-x2) > critvalues[clustersize]:
                    #   print "Critical Value 2x Fails"
                        if not tempcluster[clustersize-1] in outliers:
                            outliers.append(tempcluster[0])
                tempcluster = sorted(cluster, key=lambda atom: atom['y'])
            ...
            if(clustersize >= 14 and clustersize <= 30):
                ...
                # print "Size %d"%clustersize
                    ...
                    xn1 = tempcluster[clustersize-2]['x']
                    xn2 = tempcluster[clustersize-3]['x']
                    if (x3-x1) / (xn2-x1) > critvalues[clustersize]:
                        # print "Critical Value 1x Fails"
                            if not tempcluster[0] in outliers:
                                outliers.append(tempcluster[0])
                    if (xn-xn2) / (xn-x3) > critvalues[clustersize]:
                        #  print "Critical Value 2x Fails"
                            if not tempcluster[clustersize-1] in outliers:
                                outliers.append(tempcluster[0])
                    tempcluster = sorted(cluster, key=lambda atom: atom['y'])
    ...
    def phase3(self):
            ...
            #for i in progressbar(pdb.twlist, "Processing Waters..."):
                ...
                for atom in pdb.twlist[i]:
                    if atom['chain']=='W':
                        toupdate=atom
                    for cluster in self.master:
                        if cluster.estimate_fit(atom,1.4):
                            if cluster.res_num<bestfit:
                                bestfit=cluster.res_num
                                bestwater=atom
                if toupdate!=None:
        ...
        waterclusterpdb.write(self.stamp)
        #orgclusterpdb.write(self.stamp)
        #And write to file.
        self.write_master()
        for pdb in self.pdbs:
            pdb.write(self.stamp,2)
